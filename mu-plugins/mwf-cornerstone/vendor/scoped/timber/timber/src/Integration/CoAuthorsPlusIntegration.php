...
use Mwf\Cornerstone\Deps\CoAuthors_Plus;
use Mwf\Cornerstone\Deps\Timber\Integration\CoAuthorsPlus\CoAuthorsPlusUser;
use WP_User;
/** @internal */
class CoAuthorsPlusIntegration implements IntegrationInterface
{
    ...
    {
        \add_filter('mwf_cornerstone_timber/post/authors', [$this, 'authors'], 10, 2);
        \add_filter('mwf_cornerstone_timber/user/class', function ($class, WP_User $user) {
    ...
    {
        $authors = [];
        $cauthors = \Mwf\Cornerstone\Deps\get_coauthors($post->ID);
        foreach ($cauthors as $author) {
            $uid = $this->get_user_uid($author);
            if ($uid) {
                $authors[] = \Mwf\Cornerstone\Deps\Timber\Timber::get_user($uid);
            } else {
                $wp_user = new WP_User($author);
                $user = \Mwf\Cornerstone\Deps\Timber\Timber::get_user($wp_user);
