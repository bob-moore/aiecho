...
class Helpers
{
    ...
    public static function classUses(string|object $instance_or_class, string $needle) : bool
    {
        ...
        return match (\true) {
            ...
            \trait_exists($needle) => self::uses($class_name, $needle),
            default => \false,
        };
    }
    /**
    ...
    {
        $base = \str_replace(\trailingslashit(\WP_PLUGIN_DIR), '', __DIR__);
        $parts = \explode('/', $base);
        return \trailingslashit(\WP_PLUGIN_DIR) . $parts[0] . '/';
    }
    /**
    ...
    public static function packageType() : string
    {
        ...
        return match (\true) {
            ...
            \str_contains($path, \get_stylesheet_directory()) && !\str_contains($path, \get_template_directory()) => 'child-theme',
    ...
    {
        return match ($package_type ?? self::packageType()) {
            'plugin' => \plugin_dir_url(\trailingslashit(\trailingslashit($dir ?? self::getDefaultDir($package_type)) . '*.php')),
    ...
    {
        if (\function_exists('Mwf\\Cornerstone\\Deps\\dump')) {
            \Mwf\Cornerstone\Deps\dump($item);
        }
    }
}
