 ...
 * @since   1.0.0
 */
namespace Mwf\Cornerstone\Deps\Devkit\WPCore;

use Mwf\Cornerstone\Deps\Devkit\WPCore\DI\ContainerBuilder;
...
class Main extends Abstracts\Mountable implements Interfaces\Main, Interfaces\Controller
{
    ...
    {
        $type = \strtolower($config['type'] ?? Helpers::packageType());
        $package = $config['package'] ?? static::PACKAGE ?? Helpers::slugify(\basename(Helpers::getDefaultDir($type)));
        $app_dir = \untrailingslashit($config['dir'] ?? Helpers::getDefaultDir($type));
        $app_url = \untrailingslashit($config['url'] ?? Helpers::getDefaultUrl($app_dir));
        $assets = ['dir' => \untrailingslashit(\ltrim((\is_string($config['assets']['dir'] ?? \false) ? $config['assets']['dir'] : null) ?? (\is_string($config['assets'] ?? \false) ? \trim($config['assets']) : 'dist'), '/')), 'url' => ContainerBuilder::string('{config.url}/{config.assets.dir}')];
        $views = ['dir' => \untrailingslashit(\ltrim((\is_string($config['views']['dir'] ?? \false) ? \trim($config['views']['dir']) : null) ?? (\is_string($config['views'] ?? \false) ? $config['views'] : 'views'), '/'))];
        $this->config = \array_merge($config, ['type' => $type, 'package' => $package, 'dir' => $app_dir, 'url' => $app_url, 'assets' => $assets, 'views' => $views]);
    }
    /**
    ...
    protected function buildContainer() : ContainerInterface
    {
        ...
        $container_builder->useAttributes(\true);
        $container_builder->addDefinitions(['config' => ContainerBuilder::array($this->config)], [Controllers\Handlers::class => ContainerBuilder::autowire()], static::getServiceDefinitions());
    ...
    public static function mount(array $config = []) : static
    {
        ...
        $instance->setContainer($instance->getContainer() ?? $instance->buildContainer());
        $instance->onMount();
        return $instance;
    }
    /**
