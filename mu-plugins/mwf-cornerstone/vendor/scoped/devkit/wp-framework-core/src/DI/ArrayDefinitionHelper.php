...
use Mwf\Cornerstone\Deps\DI\Definition\ArrayDefinition, Mwf\Cornerstone\Deps\DI\Definition\Helper;
/**
    ...
    protected function dotNotationValues(mixed $definitions, string $key = '', int $iterations = 0) : mixed
    {
        ...
        if (\is_array($definitions)) {
            ...
            foreach ($definitions as $nested_key => $definition) {
                $unnested_definitions += $this->dotNotationValues($definition, !empty($key) ? $key . '.' . $nested_key : $nested_key, $iterations);
            }
        } else {
