 ...
 * @since   1.0.0
 */
namespace Mwf\ChildTheme\Deps\Devkit\WPCore\Traits\Uses;

use Mwf\ChildTheme\Deps\DI\Attribute\Inject, Mwf\ChildTheme\Deps\Devkit\WPCore\Interfaces;
/**
 * Style user Trait
 *
...
trait Styles
{
     ...
     */
    #[Inject]
    public function setStyleHandler(#[Inject(Interfaces\Handlers\Styles::class)] Interfaces\Handlers\Styles $style_handler = null) : void
    {
     ...
     * @return void
     */
    public function enqueueStyle(string $handle, string $path, array $dependencies = [], string $version = null, $screens = 'all') : void
    {
        if (isset($this->style_handler) && !\is_null($this->style_handler)) {
            $this->style_handler->enqueue($handle, $path, $dependencies, $version, $screens);
        }
    }
}
