...
use Mwf\ChildTheme\Deps\Timber\Image\Operation;
/**
 * Class ImageHelper
 *
...
class ImageHelper
{
    ...
    public static function get_sideloaded_file_loc($file)
    {
         ...
         * add_filter('theme_name_timber/sideload_image/basename', function ($basename, $path_parts) {
         ...
         */
        $basename = \apply_filters('theme_name_timber/sideload_image/basename', $basename, $path_parts);
        $ext = 'jpg';
    ...
    public static function set_sideload_image_upload_dir(array $upload)
    {
         ...
         * add_filter('theme_name_timber/sideload_image/subdir', function( $subdir ) {
         *     return 'sideloaded';
         * } );
         *
         * // Disable subdirectory used for sideloaded images.
         * add_filter('theme_name_timber/sideload_image/subdir', '__return_false' );
         * ```
         *
         ...
         *                       `external`.
         */
        $subdir = \apply_filters('theme_name_timber/sideload_image/subdir', $subdir);
        if (!empty($subdir)) {
    ...
    public static function analyze_url(string $url) : array
    {
         ...
         */
        $result = \apply_filters('theme_name_timber/image_helper/pre_analyze_url', null, $url);
        if (null === $result) {
         ...
         * @param string               $url  The URL pointing to an image.
         */
        return \apply_filters('theme_name_timber/image_helper/analyze_url', $result, $url);
    }
    /**
    ...
    public static function theme_url_to_dir(string $src) : string
    {
         ...
         */
        $path = \apply_filters('theme_name_timber/image_helper/pre_theme_url_to_dir', null, $src);
        if (null === $path) {
         ...
         */
        return \apply_filters('theme_name_timber/image_helper/theme_url_to_dir', $path, $src);
    }
    /**
     * Converts a URL located in a theme directory into the raw file path.
     *
    ...
    private static function _get_file_url($base, $subdir, $filename, $absolute)
    {
        ...
        if (!$absolute) {
            $home = \home_url();
            $home = \apply_filters('theme_name_timber/image_helper/_get_file_url/home_url', $home);
    ...
    private static function _operate($src, $op, $force = \false)
    {
        ...
        $allow_fs_write = \apply_filters('theme_name_timber/allow_fs_write', \true);
         ...
         */
        $new_url = \apply_filters('theme_name_timber/image/new_url', $new_url);
        /**
         ...
         */
        $destination_path = \apply_filters('theme_name_timber/image/new_path', $destination_path);
        // if already exists...
    ...
    public static function is_protocol_allowed($filepath)
    {
        ...
        $protocol = isset($parsed_url['scheme']) ? \mb_strtolower($parsed_url['scheme']) : 'file';
        if (\PHP_OS_FAMILY === 'Windows' && \strlen($protocol) === 1 && \preg_match(self::WINDOWS_LOCAL_FILENAME_REGEX, $filepath)) {
            $protocol = 'file';
        }
        return \in_array($protocol, self::ALLOWED_PROTOCOLS, \true);
    }
}
