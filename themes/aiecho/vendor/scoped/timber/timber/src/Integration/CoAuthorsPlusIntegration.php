...
use Mwf\ChildTheme\Deps\CoAuthors_Plus;
use Mwf\ChildTheme\Deps\Timber\Integration\CoAuthorsPlus\CoAuthorsPlusUser;
use WP_User;
/** @internal */
class CoAuthorsPlusIntegration implements IntegrationInterface
{
    ...
    {
        \add_filter('theme_name_timber/post/authors', [$this, 'authors'], 10, 2);
        \add_filter('theme_name_timber/user/class', function ($class, WP_User $user) {
    ...
    {
        $authors = [];
        $cauthors = \Mwf\ChildTheme\Deps\get_coauthors($post->ID);
        foreach ($cauthors as $author) {
            $uid = $this->get_user_uid($author);
            if ($uid) {
                $authors[] = \Mwf\ChildTheme\Deps\Timber\Timber::get_user($uid);
            } else {
                $wp_user = new WP_User($author);
                $user = \Mwf\ChildTheme\Deps\Timber\Timber::get_user($wp_user);
