...
class AcfIntegration implements IntegrationInterface
{
    ...
    {
        \add_filter('theme_name_timber/post/pre_meta', [__CLASS__, 'post_get_meta_field'], 10, 5);
        \add_filter('theme_name_timber/post/meta_object_field', [__CLASS__, 'post_meta_object'], 10, 3);
        \add_filter('theme_name_timber/term/pre_meta', [__CLASS__, 'term_get_meta_field'], 10, 5);
        \add_filter('theme_name_timber/user/pre_meta', [__CLASS__, 'user_get_meta_field'], 10, 5);
        /**
         * Allowed a user to set a meta value
         *
         * @deprecated 2.0.0 with no replacement
         */
        \add_filter('theme_name_timber/term/meta/set', [__CLASS__, 'term_set_meta'], 10, 4);
    }
    /**
    ...
    {
        return \Mwf\ChildTheme\Deps\get_field_object($field_name, $post_id);
    }
    /**
    ...
    public static function term_set_meta($value, $field, $term_id, $term)
    {
        ...
        \Mwf\ChildTheme\Deps\update_field($field, $value, $searcher);
        return $value;
    }
    /**
    ...
    public static function transform_date_picker($value, $id, $field)
    {
        ...
        return new DateTimeImmutable(\Mwf\ChildTheme\Deps\acf_format_date($value, 'Y-m-d H:i:s'));
    }
    /**
    ...
    private static function get_meta($value, $id, $field_name, $args)
    {
        ...
        if (!$args['transform_value']) {
            return \Mwf\ChildTheme\Deps\get_field($field_name, $id, $args['format_value']);
        }
        /**
         ...
         */
        $file_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('file');
        $image_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('image');
        $gallery_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('gallery');
        $date_picker_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('date_picker');
        $date_time_picker_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('date_time_picker');
        $post_object_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('post_object');
        $relationship_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('relationship');
        $taxonomy_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('taxonomy');
        $user_field_type = \Mwf\ChildTheme\Deps\acf_get_field_type('user');
        ...
        $value = \Mwf\ChildTheme\Deps\get_field($field_name, $id, \true);
