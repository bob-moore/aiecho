...
use Mwf\ChildTheme\Deps\Relay\Relay;
...
class RedisCaster
{
    ...
    {
        $serializer = $redis->getOption(\defined('Mwf\\ChildTheme\\Deps\\Redis::OPT_SERIALIZER') ? \Redis::OPT_SERIALIZER : 1);
        ...
        $compression = \defined('Mwf\\ChildTheme\\Deps\\Redis::OPT_COMPRESSION') ? $redis->getOption(\Redis::OPT_COMPRESSION) : 0;
        ...
        $retry = \defined('Mwf\\ChildTheme\\Deps\\Redis::OPT_SCAN') ? $redis->getOption(\Redis::OPT_SCAN) : 0;
        if (\is_array($retry)) {
            foreach ($retry as &$v) {
        ...
        $options += ['TCP_KEEPALIVE' => \defined('Mwf\\ChildTheme\\Deps\\Redis::OPT_TCP_KEEPALIVE') ? $redis->getOption(\Redis::OPT_TCP_KEEPALIVE) : Relay::OPT_TCP_KEEPALIVE, 'READ_TIMEOUT' => $redis->getOption(\defined('Mwf\\ChildTheme\\Deps\\Redis::OPT_READ_TIMEOUT') ? \Redis::OPT_READ_TIMEOUT : Relay::OPT_READ_TIMEOUT), 'COMPRESSION' => $compression, 'SERIALIZER' => $serializer, 'PREFIX' => $redis->getOption(\defined('Mwf\\ChildTheme\\Deps\\Redis::OPT_PREFIX') ? \Redis::OPT_PREFIX : Relay::OPT_PREFIX), 'SCAN' => $retry];
